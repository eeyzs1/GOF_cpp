...
typedef enum class PieceColorTag
{
    BLACK,//0
    WHITE//1
}PIECECOLOR;
...
typedef struct pointTag//UNSHARED FLYWEIGHTS
{
    int x;
    int y;
...
class CPiece//SHARED FLYWEIGHTS
{
public:
...
class CGomoku : public CPiece//SHARED CONCRETE FLYWEIGHTS
{
public:
    CGomoku(PIECECOLOR color) : CPiece(color) {}
};

class CPieceFactory//FLYWEIGHT FACTORY
{
public:
...
int main()
{
    ...
    CPiece* pPiece = pPieceFactory->GetPiece(PIECECOLOR::WHITE);
    pPiece->SetPoint(POINT(2, 3));
    pCheseboard->Draw(pPiece);

    // The player2 get a black piece from the pieces bowl
    pPiece = pPieceFactory->GetPiece(PIECECOLOR::BLACK);
    pPiece->SetPoint(POINT(4, 5));
    pCheseboard->Draw(pPiece);

    // The player1 get a white piece from the pieces bowl
    pPiece = pPieceFactory->GetPiece(PIECECOLOR::WHITE);
